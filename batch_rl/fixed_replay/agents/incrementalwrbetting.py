class IncrementalWRBetting:
    ...
    def bet(self, v):
        ...
        return min(self.taumax, max(0, mean / (mean + var) if var > 0 else 1 if mean >= 0 else 0))

    def findv(self):
        # TODO: this is buggy (but worked ...)
        from math import sqrt

        meanwr = self.sumwr / max(1, self.n)
        varwr = self.sumwrsq / max(1, self.n) - meanwr**2
        vadj = max(0, min(1, meanwr - 3 * varwr / sqrt(max(1, self.n))))
        return vadj
